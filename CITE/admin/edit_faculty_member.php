...
if(isset($_GET['id']) && is_numeric($_GET['id']))
{
	...
	if(isset($_POST['name'])) {
		...
		$reject = false;
		$query_arr = array();
		if (!($reject = $reject && (Validations::email($_POST, 'email') && $requirements['email'])))
			$user->set_email($_POST['email']);

		if (!($reject = $reject && (Validations::string($_POST, 'name') && $requirements['name'])))
			$user->set_name($_POST['name']);

		if (!($reject = $reject && (Validations::string($_POST, 'l_name') && $requirements['l_name'])))
			$user->set_last_name($_POST['l_name']);

		if (!($reject = $reject && (Validations::division($_POST, 'division') && $requirements['division'])))
			$user->set_division($_POST['division']);
		
		if (!($reject = $reject && (Validations::phone($_POST, 'phone') && $requirements['phone'])))
			$user->set_phone($_POST['phone']);

		if (!($reject = $reject && (Validations::string($_POST, 'office_address') && $requirements['office_address'])))
			$user->set_office_address($_POST['office_address']);
		
		if (!($reject = $reject && (Validations::string($_POST, 'designation') && $requirements['designation'])))
			$user->set_designation($_POST['designation']);
		
		if (!($reject = $reject && (Validations::string($_POST, 'overview') && $requirements['overview'])))
		...
		if ($user->get_permissions() > 2)
			$user->set_permissions(intval($_POST['permission']));
	 
		if($image_status === TRUE && move_uploaded_file($temp_name, $target_path)) {
			$user->set_image($target_path);
		}
...
<body>
			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-8">
						<div class="card">
						<div class="content">
							<a href="/cite/faculty.php?id=<?php echo $user->get_id(); ?>">View user profile</a>
						</div></div>
						<?php echo $pat; ?>
						<div class="card">
							<div class="header">
	...
	function validateForm(event){
		...
		if(designation.replace(/\s{2,}/g, ' ').split(" ").length>50  || requiredFields.indexOf('designation') !== -1 && designation == '') {
			...
			alerts.push("Please enter a valid designation.");
		}
		...
		if(office_address.length > 140 || requiredFields.indexOf('office_address') !== -1 && office_address == '') {
        	...
        	alerts.push("Please enter a valid office address.");
		}
		...
		//if (degree1.length > 0) {
			...
			if(degree1.length > 100  || requiredFields.indexOf('degree1') !== -1 && degree1 == '') {
				...
				alerts.push("Please enter a valid degree 1 name.");
			}
			...
			if(degree1_school.length > 100  || requiredFields.indexOf('degree1_school') !== -1 && degree1_school == '') {
				...
				alerts.push("Please enter a valid degree 1 school name.");
			}
		...
		//if (degree2.length > 0) {
			...
			if(degree2.length > 100  || requiredFields.indexOf('degree2') !== -1 &&degree2 == '') {
				...
				alerts.push("Please enter a valid degree 2 name.");
			}
			...
			if(degree2_school.length > 100  || requiredFields.indexOf('degree2_school') !== -1 &&degree2_school == '') {
				...
				alerts.push("Please enter a valid degree 2 school name.");
			}
		...
		//if (degree3.length > 0) {
			...
			if(degree3.length > 100  || requiredFields.indexOf('degree3') !== -1 && degree3.length < 1){
				...
				alerts.push("Please enter a valid degree 3 name.");
			}
			...
			if(degree3_school.length > 100  || (requiredFields.indexOf('degree3_school') !== -1 && degree3_school.length < 1)){
				...
				alerts.push("Please enter a valid degree 3 school name.");
			}
		...
		if(overview.replace(/\s{2,}/g, ' ').split(" ").length > 500 || requiredFields.indexOf('overview') !== -1 && overview == ''){
			...
			alerts.push("Please enter a valid overview.");
		}
